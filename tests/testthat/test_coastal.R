...
source_gpkg <- list.files(pattern = "coastal.gpkg$",
                          recursive = TRUE,
                          full.names = TRUE)
...
source_fdr <- list.files(pattern = "coastal_fdr.tif$",
                         recursive = TRUE,
                         full.names = TRUE)
source_fac <- list.files(pattern = "coastal_fac.tif$",
                         recursive = TRUE,
                         full.names = TRUE)
min_da_km <- 10
...
test_that("basic coastal aggregation", {
  ...
  nhd <- dplyr::filter(
    nhd,
    COMID %in% nhdplusTools::prepare_nhdplus(nhd, 0, 0, 0, FALSE, 
                                             skip_toCOMID = TRUE, 
                                             warn = FALSE)$COMID
  ) |> 
    select(-id)
  ...
  networks <- do.call(c,
                      sapply(dplyr::filter(nhd, TerminalFl == 1)$COMID, function(x, nhd)
                        nhdplusTools::get_UT(network = nhd, x), nhd = nhd))
  ...
  little_terminal <- dplyr::filter(
    nhd,
    TerminalPa %in%
      dplyr::filter(nhd_outlets, TotDASqKM <= min_da_km &
  ...
  avoid <- dplyr::filter(nhd, (sqrt(AreaSqKM) / LENGTHKM) > 3 &
                           AreaSqKM > 1)
  ...
  refactor_nhdplus(
    ...
    exclude_cats = unique(c(
      avoid$COMID, outlets$COMID, little_terminal$COMID
    )),
    warn = FALSE
  )
  ...
  divides <- reconcile_catchment_divides(
    catchment = select(cats, -id),
