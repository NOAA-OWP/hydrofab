...
aggregate_along_mainstems = function(network_list,
  ...
  hyaggregate_log("INFO",
                  glue("ideal_size_sqkm --> {ideal_size_sqkm}"),
                  verbose)
  ...
  index_table = fline %>%
    ...
    ungroup() %>%
    select(set, id, toid, levelpathid, hydroseq, member_comid, poi_id, n)
  ...
  v$lookup = 
    index_table |> 
    group_by(set) |> 
    mutate(flowline_id = paste(id, collapse = ","),
           member_comid = paste(member_comid, collapse = ",")) |> 
    ungroup() |> 
    select(id = set, flowline_id, member_comid ) |> 
    distinct()
  
  hyaggregate_log(
    "SUCCESS",
    glue(
      "Merged to idealized catchment size of {ideal_size_sqkm} sqkm: {nrow(network_list$flowpaths) - nrow(v$flowpaths)} features removed"
    ),
    verbose
  )
  ...
  if (!is.null(cache_file)) {
    ...
    write_hydrofabric(tmp, cache_file, verbose, enforce_dm = FALSE)
...
cs_group <- function(areas,
  ...
  o2 = lapply(1:length(sub_areas), function(i) {
    pinch_sides(x = sub_areas[[i]],
                ind = o1[[i]],
                thres = amin)
  })
  
  o3 = lapply(1:length(sub_lengths), function(i) {
    pinch_sides(x = sub_lengths[[i]],
                ind = o2[[i]],
                thres = lmin)
  })
  
  o4 = lapply(1:length(sub_areas), function(i) {
    middle_massage(x = sub_areas[[i]],
                   ind = o3[[i]],
                   thres = amin)
  })
  
  o5 = lapply(1:length(sub_lengths), function(i) {
    middle_massage(x = sub_lengths[[i]],
                   ind = o4[[i]],
                   thres = lmin)
  })
  
  for (i in 1:length(o5)) {
    o5[[i]] = o5[[i]] + 1e9 * i
  }
...
aggregate_sets = function(network_list, index_table) {
  set_topo = index_table %>%
    ...
    arrange(hydroseq) %>%
    select(set, id, toid, levelpathid, poi_id, hydroseq, member_comid) %>%
    ungroup()
